...
>
> 7. **Inspection minigames (hiders stay still while being held).** When the seeker picks a hider up (inspects it), the hider must pass a short, mobile-friendly minigame on their phone to "hold still". Each minigame **shows its instructions first (a ~1.5s intro)**, then plays. Passing keeps the prop calm and still held (looking like a decoy); **failing makes it shake, play a surprised yelp, and pop out of the seeker's hand** — revealing it was alive, but not eliminating it. The minigame gets harder each time the same hider is re-inspected. The minigame **runs on the phone** for instant input; the host picks which one (at random, unless `hiders.inspection_minigame` forces one), enforces a safety deadline (idle = auto-fail so you can't just do nothing), and reacts to the reported pass/fail. There are **15 minigames** so pickups stay fresh; the framework and full list live in **`MINIGAMES.md`**. This realizes and expands §6.5's pickup "panic wiggle" idea.
>
