...
>
> 7. **Inspection minigames (hiders stay still while being held).** When the seeker picks a hider up (inspects it), the hider must pass a short, mobile-friendly minigame on their phone to "hold still". Passing keeps the prop calm and still held (looking like a decoy); **failing makes it shake, play a surprised yelp, and pop out of the seeker's hand** — revealing it was alive, but not eliminating it. The minigame gets harder each time the same hider is re-inspected in a round. Rules are authoritative on the host (`hiders.inspection_minigame_enabled`, `hiders.inspection_minigame`); the phone only renders the state and sends a normalized input. The framework and the list of implemented minigames live in **`MINIGAMES.md`**; the first shipped minigame is **Steady Hands** (drag to keep a marker in a shrinking safe zone). This realizes and expands §6.5's pickup "panic wiggle" idea.
>
