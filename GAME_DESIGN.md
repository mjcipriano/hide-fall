...
>
> 1. **No blackout phase, and no separate object-rain phase.** The round flow is lobby → (optional room setup) → seek. When the hunt starts, all props and hiders spawn up near the ceiling and **rain down and settle live during the hunt** — there is no vision-darkening stage and no dead 10-second "props falling" wait. Hiders are airborne (uncontrollable) for the ~1 second it takes them to land, then steer normally. `round.blackout_seconds` and `round.object_rain_seconds` no longer exist; the `object_rain` phase is gone. This supersedes §3.2's "Object rain phase: 10 seconds", §4.1 steps 3–4, and §6.3/§6.4.
...
> 4. **Hider abilities.** `Freeze` was replaced on phones by `Dash` (burst of speed) and `Mimic` (instantly copy an adjacent decoy's shape/color/pattern), both with cooldowns. Added in v0.3.4: `Earthquake` (one use per round: every loose prop is hurled in a random direction into the air and tumbles back down) and `Ping` (a sub-second taunt jingle the seeker hears spatially from the hider's prop; each player's jingle is unique, seeded from their player id; short cooldown).
> 5. **Game modes** (`round.mode`, selectable from the wrist menu). `one_shot` (default): a found hider is eliminated. `endless_hiders`: every shot object is destroyed; a found hider respawns into a random surviving decoy body and keeps playing, so the prop pile shrinks with every shot. Hiders score on movement/survival, the seeker on total finds. The round ends when no bodies remain, all hiders are out, or the seeker spends their last shot.
>
> 7. **Inspection minigames (hiders stay still while being held).** When the seeker picks a hider up (inspects it), the hider must pass a short, mobile-friendly minigame on their phone to "hold still". Each minigame **shows its instructions first (a ~1.5s intro)**, then plays. Passing keeps the prop calm and still held (looking like a decoy); **failing makes it shake, play a surprised yelp, and pop out of the seeker's hand** — revealing it was alive, but not eliminating it. The minigame gets harder each time the same hider is re-inspected. The minigame **runs on the phone** for instant input; the host picks which one (at random, unless `hiders.inspection_minigame` forces one), enforces a safety deadline (idle = auto-fail so you can't just do nothing), and reacts to the reported pass/fail. There are **45 minigames** so pickups stay fresh — quick action games (tap/hold/drag) plus word & quiz games (odd-one-out, rhymes, opposites, spelling, unscramble, Stroop, quick maths, memory, Simon, etc.) via a labelled choice-button grid; the framework and full list live in **`MINIGAMES.md`**. This realizes and expands §6.5's pickup "panic wiggle" idea.
>
> 6. **No disguise cooldowns by default.** `hiders.shape_change_cooldown`, `hiders.color_change_cooldown`, and `hiders.mimic_cooldown_seconds` all default to `0` — hiders may change shape/color and mimic as often and as fast as they want. The cooldown mechanic still exists and can be re-armed from the wrist menu (Shape/Color sliders include a `0`/off stop) for hosts who want the old §6.5 pacing. This supersedes the "Shape change cooldown: default 12 seconds / Color change cooldown: default 6 seconds" numbers in §6.5.
