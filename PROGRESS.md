...
## 2026-07-02 Follow-up: Disguise cooldowns off by default

User wanted mobile hiders to change shape/color and mimic with no delay — as fast and as often as they like — and reported the wrist "Timer" (Off) still seemed to end rounds on a clock.

- Defaults changed to `0`: `hiders.shape_change_cooldown`, `hiders.color_change_cooldown`, and `hiders.mimic_cooldown_seconds`, in both `game/content/settings/default.json` and the `GameConfig._apply_defaults` fallback. A `0` cooldown means `_try_change_shape`/`_try_change_color`/`_try_mimic` re-arm to `0`, so the next change is immediately allowed. The cooldown mechanic is unchanged, just no longer gating by default.
- `xr_settings_menu.gd`: Shape slider values are now `[0, 4, 8, 12, 18]` and Color `[0, 2, 4, 6, 10]` so the new `0` default renders and a host can re-arm the old pacing from the wrist menu.
- `GAME_DESIGN.md`: added amendment #6 recording no-cooldown-by-default and that it supersedes the §6.5 "12s / 6s" numbers.
- Tests: `_test_hider_input_and_cooldowns` now asserts repeat shape/color changes apply immediately with the default config, then sets an explicit cooldown to prove the gate still blocks repeats; the mimic ability test sets a configured `mimic_cooldown_seconds` before asserting the cooldown arms. `make test` passes (`Godot tests passed`).
- Round audio cues added. The sim now queues two new events for the host to sound: `round_over` (once, from `_finish_round`, carries `all_hiders_found`) and `countdown_tick` (once per whole second during the last `round.countdown_tick_seconds` — default 15 — of a timed hunt, `final=true` for the last 3 seconds). Ticks only fire when `end_on_seek_timeout` is on, since an off timer has no clock. `host_prototype` synthesizes three new procedural SFX (`tick` wood-block, `tick_final` sharper/higher, `round_over` resolving C-E-G fanfare) and plays them from `_process_simulation_events`. New config key `round.countdown_tick_seconds` in `default.json` + `GameConfig` fallback. Test `_test_round_audio_cues` asserts 13–15 ticks + 3 final ticks + exactly one round-over over a 20s timed round, and zero ticks with the timer off. `make test` passes (290 PASS, 0 FAIL).
- Root-caused the "timed session" report: with the timer off the round never ends on a clock, but the wrist panel and mobile still displayed the seek countdown shrinking 90→0 (the snapshot `time_remaining` was computed from `seek_seconds - elapsed` regardless of the timer), which reads as a game-over timer. Fix: added `_display_time_remaining()` — same as `_time_remaining()` except it returns `0` during the hunt when `end_on_seek_timeout` is off — and the snapshot now sends that. Scoring still uses raw `_time_remaining()` for its fast-finish `time_bonus`. Mobile already hid a `0` seek countdown (shows "SEEKER HUNTING"); the Quest wrist panel now shows `Hunt <elapsed>` counting up instead of `Time <remaining>` during an untimed hunt. New test `_test_hunt_countdown_display` asserts snapshot `time_remaining==0` with the timer off and `>0` with it on. The 10s "props falling" countdown is intentionally kept — that phase really does end in 10s and hand off to the hunt.
- Timer report investigation: the seek-timeout round end (`hidefall_simulation.gd:252`) is gated on `round.end_on_seek_timeout`, which is `false` by default; `host_prototype._process` only calls `simulation.advance` with no separate timer. The test suite still confirms "hunt timer does not end rounds by default" and the wrist toggle "defaulting off". No timer-driven round end exists in the current source with the toggle off — the likely culprits are a stale installed APK from before the ammo/timer rework (commit `69f47fb`) or the seeker running out of ammo (`round.end_when_out_of_shots=true`), which is not a clock. Needs a device rebuild/reconfirm rather than a code change.

## 2026-07-02 Follow-up: Remove object-rain phase, menu polish, inspection minigames (v0.3.7)

- **Removed the object-rain phase.** The round now goes lobby → (optional room setup) → seek; props and hiders spawn near the ceiling and rain down/settle *live* during the hunt (the seek integrators already apply gravity), so there is no dead 10s wait. Dropped `round.object_rain_seconds`, the `object_rain` phase constant, `_integrate_object_rain`, and all phase labels/branches across sim/host/mobile; `validate_content.py` no longer requires the key. GAME_DESIGN amendment #1 updated. Tests assert props start airborne and settle after the hunt starts.
- **Wrist settings menu polish.** Added a visible aim laser (green when a row is under the dot, amber otherwise) from the controller to the panel, plus a bright on-panel dot at the hit point (`xr_settings_menu.gd` exposes `is_pointer_active`/`get_pointer_world_point`; `host_prototype._update_menu_laser`). Bumped all menu fonts ~1.5x and much crisper (font_size 6-8 → 24-28) while preserving alignment boxes and the compact panel size.
